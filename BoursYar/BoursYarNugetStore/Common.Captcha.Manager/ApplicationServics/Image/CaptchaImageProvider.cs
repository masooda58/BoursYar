...
using Common.Captcha.Manager.Config.IServices;
...
namespace Common.Captcha.Manager.ApplicationServics.Image
{
    public class CaptchaImageProvider : ICaptchaImageProvider
    {
        ...
        public byte[] DrawCaptcha(string captcha, CaptchaImageParams imageParams)
        {
            using (Bitmap baseMap = new Bitmap(imageParams.Width, imageParams.Height))
            using (Graphics graph = Graphics.FromImage(baseMap))
            {
                ...
                void DrawCaptchaCode()
                {
                    ...
                    int fontSize = GetFontSize(imageParams.Width, captcha.Length);
                    ...
                    for (int i = 0; i < captcha.Length; i++)
                    {
                        ...
                        int maxY = imageParams.Height - fontSize;
                        if (maxY < 0) maxY = 0;
                        float y = rand.Next(0, maxY);

                        graph.DrawString(captcha[i].ToString(), font, fontBrush, x, y);
                    }
                }
                ...
                void DrawDisorderLine()
                {
                    ...
                    for (int i = 0; i < rand.Next(3, 5); i++)
                    {
                        ...
                        Point startPoint = new Point(rand.Next(0, imageParams.Width), rand.Next(0, imageParams.Height));
                        Point endPoint = new Point(rand.Next(0, imageParams.Width), rand.Next(0, imageParams.Height));
